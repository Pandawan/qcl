 ...
 */
async function uninstallAfterCleanup(pkgName) {
    if (!pkgName) {
...
exports.uninstallAfterCleanup = uninstallAfterCleanup;
async function uninstallPackage(pkgName) {
    const { package_manager } = data_1.getData();
    if (package_manager === 'npm') {
        await utils_1.getAsync(`npm uninstall ${pkgName} -g`);
    }
    else if (package_manager === 'yarn') {
        await utils_1.getAsync(`sudo yarn global remove ${pkgName}`);
    }
}
