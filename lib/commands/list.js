 ...
 */
async function expireTime(installedDate, preservationTime) {
    // Calculate difference between expiry date & current date
    const diff = moment_1.default(installedDate)
        .add(preservationTime[0], preservationTime[1])
        // Use same units as preservationTime
        .diff(moment_1.default(), preservationTime[1]);
